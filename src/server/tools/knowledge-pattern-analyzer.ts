...
export interface PatternAnalysisResult {
    newPatterns: ProblemPattern[];
    updatedPatterns: ProblemPattern[];
...
class KnowledgePatternAnalyzer {
     ...
     * 按类别分析问题模式
     */
    private async analyzeCategory(category: string, problems: DatabaseSchema['problems'][]): Promise<ProblemPattern | null> {
     ...
     * 按标签组合分析问题模式
     */
    private async analyzeTagCombination(tagCombo: string, problems: DatabaseSchema['problems'][]): Promise<ProblemPattern | null> {
        const tags = tagCombo.split('|');
        if (problems.length < 2 || tags.length < 2) return null;
     ...
     * 智能总结常见解决方案
     */
    private findCommonSolutions(problems: DatabaseSchema['problems'][]): string[] {
     ...
     * 查找共同标签
     */
    private findCommonTags(problems: DatabaseSchema['problems'][]): string[] {
     ...
     * 提取预防策略
     */
    private extractPreventionStrategies(problems: DatabaseSchema['problems'][]): string[] {
        return problems
    ...
    private generatePatternDescription(
        category: string,
        problems: DatabaseSchema['problems'][],
        commonTags: string[],
     ...
     * 计算时间分散度
     */
    private calculateTimeSpread(problems: DatabaseSchema['problems'][]): number {
        if (problems.length < 2) return 0;
     ...
     * 按类别分组问题
     */
    private groupProblemsByCategory(problems: DatabaseSchema['problems'][]): Record<string, DatabaseSchema['problems'][]> {
        const groups: Record<string, DatabaseSchema['problems'][]> = {};
     ...
     * 按标签组合分组问题
     */
    private groupProblemsByTagCombinations(problems: DatabaseSchema['problems'][]): Record<string, DatabaseSchema['problems'][]> {
        const groups: Record<string, DatabaseSchema['problems'][]> = {};
     ...
     * 按解决方案类型分组问题
     */
    private groupProblemsBySolutionType(problems: DatabaseSchema['problems'][]): Record<string, DatabaseSchema['problems'][]> {
        const groups: Record<string, DatabaseSchema['problems'][]> = {};
     ...
     * 获取所有问题
     */
    private async getAllProblems(): Promise<DatabaseSchema['problems'][]> {
        return await this.dbManager.query(`
            SELECT * FROM problems 
            WHERE created_at >= datetime('now', '-30 days')
            ORDER BY created_at DESC
        `) as DatabaseSchema['problems'][];
    }

    /**
     * 获取现有模式
     */
    private async getExistingPatterns(): Promise<ProblemPattern[]> {
    ...
    private async updatePattern(
        existingPattern: ProblemPattern,
        allProblems: DatabaseSchema['problems'][]
    ): Promise<ProblemPattern | null> {
        // 找到属于此模式的新问题
        const relevantProblems = allProblems.filter(problem =>
            existingPattern.categories.includes(problem.category || '')
        );
     ...
     * 计算分析置信度
     */
    private calculateConfidenceScore(result: PatternAnalysisResult): number {
        let confidence = 0;
     ...
     * 获取最重要的类别
     */
    private getTopCategories(patterns: ProblemPattern[]): string[] {
     ...
     * 保存模式到数据库
     */
    async savePattern(pattern: ProblemPattern): Promise<number> {
     ...
     * 更新现有模式
     */
    async updatePatternInDB(pattern: ProblemPattern): Promise<boolean> {
...
export default KnowledgePatternAnalyzer;
