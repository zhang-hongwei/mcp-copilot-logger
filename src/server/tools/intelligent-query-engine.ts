...
import { DatabaseSchema, SearchResult } from '../../types/database-schema';
...
export interface SimilarProblem {
    ...
    matchType: 'semantic' | 'keyword' | 'solution_pattern';
    explanation: string;
}
...
class IntelligentQueryEngine {
     ...
     * 关键词搜索（基于现有的全文搜索）
     */
    private async keywordSearch(options: QueryOptions): Promise<SearchResult[]> {
        // 使用现有的全文搜索功能
    ...
    private async semanticSearch(options: QueryOptions): Promise<SearchResult[]> {
        ...
        // 查询相似向量
        const similarProblems = await this.findSimilarEmbeddings(
            queryEmbedding,
            options.limit || 10,
     ...
     * 混合搜索（关键词 + 语义）
     */
    private async hybridSearch(options: QueryOptions): Promise<SearchResult[]> {
     ...
     * 查找相似问题
     */
    async findSimilarProblems(problemId: string, limit: number = 5): Promise<SimilarProblem[]> {
        // 基于现有的问题相似度表
        const similarityRecords = await this.dbManager.query(`
            ...
            LIMIT ?
        `, [problemId, limit]) as any[];

        return similarityRecords.map((record: any) => ({
            problem: record,
     ...
     * 为 Copilot 生成增强上下文
     */
    async generateCopilotContext(query: string, sessionId?: string): Promise<string> {
    ...
    private async findSimilarEmbeddings(
        ...
        limit: number,
        minSimilarity: number
    ): Promise<{ problemId: string, similarity: number }[]> {
        try {
            // 查询数据库中的所有嵌入向量
            ...
            const results: { problemId: string, similarity: number }[] = [];
     ...
     * 其他辅助方法...
     */
    private calculateKeywordRelevance(query: string, problem: DatabaseSchema['problems']): number {
        // 实现关键词相关度计算
    ...
    private getMatchedFields(query: string, problem: DatabaseSchema['problems']): string[] {
        const fields: string[] = [];
        const queryLower = query.toLowerCase();

        if (problem.title.toLowerCase().includes(queryLower)) fields.push('title');
        if (problem.description.toLowerCase().includes(queryLower)) fields.push('description');
        if (problem.final_solution.toLowerCase().includes(queryLower)) fields.push('solution');

        return fields;
    }

    private generateSnippet(query: string, problem: DatabaseSchema['problems']): string {
        // 生成包含查询词的文本片段
    ...
    private generateSemanticSnippet(problem: DatabaseSchema['problems'], query: string): string {
        // 为语义搜索生成更智能的摘要
        return `${problem.title} - ${problem.description.substring(0, 100)}...`;
    }

    private mergeAndRerankResults(keywordResults: SearchResult[], semanticResults: SearchResult[]): SearchResult[] {
    ...
    private async recordQueryHistory(options: QueryOptions, results: SearchResult[]): Promise<void> {
        // 记录查询历史用于分析和优化
    ...
    private async getRelatedPatterns(results: SearchResult[]): Promise<any[]> {
        // 基于搜索结果找出相关的知识模式
        ...
        return this.dbManager.query(`
            ...
            LIMIT 3
        `, [`%${categories[0]}%`, `%${categories[1] || categories[0]}%`]) as Promise<any[]>;
    }
    ...
    private generateSimilarityExplanation(record: any): string {
    ...
    private async recordCopilotEnhancement(
        sessionId: string,
        query: string,
        context: string,
