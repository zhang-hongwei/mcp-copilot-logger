...
import { DatabaseSchema, DatabaseOperations, ProblemFilters, DatabaseStatistics, SearchResult } from '../../types/database-schema';
...
export class DatabaseManager implements DatabaseOperations {
    ...
    public async createProblem(problem: Partial<DatabaseSchema['problems']>): Promise<string> {
        if (!this.isInitialized) {
        ...
        try {
            ...
            this.db!.prepare(stmt).run(
                id, problem.timestamp || now, problem.title || '',
                problem.description || '', problem.category || 'general',
                problem.priority || 'medium', problem.context || '',
                problem.code_before, problem.code_after, problem.code_language,
                problem.error_phenomenon || '', problem.final_solution || '',
                problem.solution_type || 'other', problem.value_score || 5,
                problem.value_explanation || '', problem.learning_outcome || '',
                problem.prevention_strategy, (problem.is_worth_exporting ?? true) ? 1 : 0,
                problem.export_notes, now, now
            );
    ...
    public async updateProblem(id: string, updates: Partial<DatabaseSchema['problems']>): Promise<boolean> {
        if (!this.isInitialized) {
            await this.initialize();
        }
    ...
    public async getProblem(id: string): Promise<DatabaseSchema['problems'] | null> {
        if (!this.isInitialized) {
            await this.initialize();
        }
        ...
        try {
            ...
            return result ? result as DatabaseSchema['problems'] : null;
        } catch (error) {
    ...
    public async searchProblems(query: string): Promise<DatabaseSchema['problems'][]> {
        if (!this.isInitialized) {
            await this.initialize();
        }
        ...
        try {
            ...
            const ftsResults = this.db!.prepare(ftsStmt).all(query) as any[];
            ...
            if (ftsResults.length > 0) {
                ...
                `;
                return this.db!.prepare(problemStmt).all(...problemIds) as DatabaseSchema['problems'][];
            } else {
                // 如果FTS没有结果，回退到LIKE搜索
                const fallbackStmt = `
                    SELECT * FROM problems 
                ...
                return this.db!.prepare(fallbackStmt).all(likeQuery, likeQuery, likeQuery) as DatabaseSchema['problems'][];
            }
        } catch (error) {
    ...
    public async getProblemsWithFilters(filters: ProblemFilters): Promise<DatabaseSchema['problems'][]> {
        if (!this.isInitialized) {
            await this.initialize();
        }
        ...
        try {
            ...
            return this.db!.prepare(stmt).all(...params) as DatabaseSchema['problems'][];
        } catch (error) {
    ...
    public async getRecentProblems(limit: number): Promise<DatabaseSchema['problems'][]> {
        if (!this.isInitialized) {
            await this.initialize();
        }

        try {
            return this.db!.prepare('SELECT * FROM problems ORDER BY created_at DESC LIMIT ?').all(limit) as DatabaseSchema['problems'][];
        } catch (error) {
    ...
    public async getSimilarProblems(problemId: string): Promise<DatabaseSchema['problems'][]>;
    public async getSimilarProblems(
        problemId: string,
        minSimilarity: number,
        limit: number
    ): Promise<Array<{ problem: DatabaseSchema['problems'], similarity: number }>>;
    public async getSimilarProblems(
        problemId: string,
        minSimilarity?: number,
        limit?: number
    ): Promise<DatabaseSchema['problems'][] | Array<{ problem: DatabaseSchema['problems'], similarity: number }>> {
        if (!this.isInitialized) {
            await this.initialize();
        }
        ...
        try {
            if (minSimilarity !== undefined && limit !== undefined) {
                ...
                return this.db!.prepare(stmt).all(problemId, problemId, problemId, problemId) as DatabaseSchema['problems'][];
            }
        } catch (error) {
     ...
     * 智能获取相似问题（基于向量相似度）
     */
    public async getSimilarProblemsWithScore(
        problemId: string,
        ...
        try {
            ...
            const results = this.db!.prepare(stmt).all(problemId, minSimilarity, limit) as any[];
            return results.map(row => ({
                problem: row as DatabaseSchema['problems'],
    ...
    // 标签操作方法
    public async createTag(name: string, color?: string, description?: string): Promise<number> {
        if (!this.isInitialized) {
            await this.initialize();
        }
    ...
    public async getOrCreateTag(name: string): Promise<number> {
        ...
        try {
            let tag = this.db!.prepare('SELECT id FROM tags WHERE name = ?').get(name) as any;
            if (!tag) {
    ...
    public async addTagToProblem(problemId: string, tagId: number): Promise<boolean> {
        if (!this.isInitialized) {
            await this.initialize();
        }
    ...
    public async removeTagFromProblem(problemId: string, tagId: number): Promise<boolean> {
        if (!this.isInitialized) {
            await this.initialize();
        }
    ...
    // 尝试记录操作
    public async addAttempt(problemId: string, action: string, result: string): Promise<number> {
        if (!this.isInitialized) {
            await this.initialize();
        }
        ...
        try {
            const now = new Date().toISOString();
            // 获取下一个序号
            const maxOrder = this.db!.prepare('SELECT MAX(sequence_order) as max_order FROM attempts WHERE problem_id = ?').get(problemId) as any;
    ...
    public async getAttempts(problemId: string): Promise<DatabaseSchema['attempts'][]> {
        if (!this.isInitialized) {
            await this.initialize();
        }

        try {
            return this.db!.prepare('SELECT * FROM attempts WHERE problem_id = ? ORDER BY sequence_order').all(problemId) as DatabaseSchema['attempts'][];
        } catch (error) {
    ...
    // 其他操作方法的Mock实现...
    public async addProblemRelation(sourceId: string, targetId: string, type: string, description?: string): Promise<number> {
        console.log(`Adding relation from ${sourceId} to ${targetId}`);
        return 1;
    }

    public async getProblemRelations(problemId: string): Promise<DatabaseSchema['problem_relations'][]> {
        console.log(`Getting relations for problem ${problemId}`);
        return [];
    }

    public async addReference(problemId: string, url: string, title?: string, description?: string): Promise<number> {
        console.log(`Adding reference to problem ${problemId}`);
        return 1;
    }

    public async getReferences(problemId: string): Promise<DatabaseSchema['problem_references'][]> {
        console.log(`Getting references for problem ${problemId}`);
        return [];
    }

    public async markAsSynced(problemId: string, obsidianPath: string): Promise<boolean> {
        console.log(`Marking problem ${problemId} as synced to ${obsidianPath}`);
        return true;
    }

    public async getPendingSyncProblems(): Promise<DatabaseSchema['problems'][]> {
        console.log('Getting pending sync problems');
        return [];
    }

    public async logSyncOperation(operation: string, targetType: string, targetId: string, status: string, error?: string): Promise<number> {
     ...
     * 存储问题向量嵌入
     */
    public async storeProblemEmbedding(
        problemId: string,
        embedding: number[],
     ...
     * 获取问题向量嵌入
     */
    public async getProblemEmbedding(problemId: string, model: string = 'text-embedding-ada-002'): Promise<number[] | null> {
        if (!this.isInitialized) {
            await this.initialize();
        }
     ...
     * 记录查询历史
     */
    public async recordQueryHistory(
        queryText: string,
     ...
     * 存储问题相似度
     */
    public async storeProblemSimilarity(
        sourceProblemId: string,
        targetProblemId: string,
        similarityScore: number,
        similarityType: 'semantic' | 'keyword' | 'solution_pattern'
    ): Promise<boolean> {
        if (!this.isInitialized) {
            await this.initialize();
        }
     ...
     * 存储知识模式
     */
    public async storeKnowledgePattern(pattern: {
        name: string;
        description: string;
        categories: string[];
        tags: string[];
        ...
        try {
            ...
            const result = stmt.run(
                ...
                JSON.stringify(pattern.solutions),
     ...
     * 记录 Copilot 增强会话
     */
    public async recordCopilotEnhancement(enhancement: {
        sessionId: string;
        originalQuery: string;
        enhancedContext?: string;
        matchedProblems: string[];
        enhancementType: 'similar_cases' | 'prevention_tips' | 'best_practices';
        ...
        try {
            ...
            const result = stmt.run(
                ...
                enhancement.enhancedContext || null,
                matchedProblemsJson,
                enhancement.enhancementType,
                enhancement.responseImproved ? 1 : 0,
                enhancement.satisfactionScore || null,
                now
            );
    ...
    public async getKnowledgeStats(): Promise<{
        ...
        try {
            ...
            const totalEmbeddings = this.db!.prepare('SELECT COUNT(*) as count FROM problem_embeddings').get() as any;
            const totalPatterns = this.db!.prepare('SELECT COUNT(*) as count FROM knowledge_patterns').get() as any;
            const recentQueries = this.db!.prepare('SELECT COUNT(*) as count FROM query_history WHERE created_at > datetime("now", "-7 days")').get() as any;
            const avgSimilarity = this.db!.prepare('SELECT AVG(similarity_score) as avg FROM problem_similarities').get() as any;
